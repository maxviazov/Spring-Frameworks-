...
# What is the Spring repository?
The Spring repository is an [Artifactory](http://www.jfrog.com) instance hosted by JFrog on behalf of Spring at http://repo.spring.io.  Artifactory is a sophisticated _repository manager_ and it is used to host all Spring open source project artifacts as well as their transitive dependencies.  It can be browsed and searched directly via the web at http://repo.spring.io, and accessed by Maven, Gradle and Ivy as well as any other build system capable working against Maven/Ivy repository layouts.

Spring project teams use repo.spring.io as a single source for resolving all project dependencies and storing all project artifacts.  Users of Spring projects can use repo.spring.io to resolve Spring artifacts and their dependencies.

***
<a name="wiki-available_repositories"/>
# What repositories are available?
repo.spring.io hosts three categories of repositories: _local_, _remote_ and _virtual_.  The complete list of repositories can be browsed [here](http://repo.spring.io/webapp/simplebrowserroot.html); below we describe the most important among these these.

**Local repositories** typically host artifacts produced directly by Spring projects.  The three main local repositories are:

* [libs-snapshot-local](http://repo.spring.io/webapp/browserepo.html?pathId=libs-snapshot-local%3A): BUILD-SNAPSHOT (development) versions
* [libs-milestone-local](http://repo.spring.io/webapp/browserepo.html?pathId=libs-milestone-local%3A): M* (milestone) and RC (release candidate) versions
* [libs-release-local](http://repo.spring.io/webapp/browserepo.html?pathId=libs-release-local%3A): RELEASE (generally available) versions

_Note: see [[spring artifact versioning|Downloading Spring artifacts#wiki-artifact_versioning]] for more information_

There also exist [plugins-snapshot-local](http://repo.spring.io/webapp/browserepo.html?pathId=plugins-snapshot-local%3A) and [plugins-release-local](http://repo.spring.io/webapp/browserepo.html?pathId=plugins-release-local%3A) repositories, used for hosting custom build system plugins, e.g. the [docbook-reference-plugin](https://github.com/SpringSource/gradle-plugins).
...
Three additional important repositories are [libs-snapshot-s3-cache](http://repo.spring.io/webapp/browserepo.html?pathId=libs-snapshot-s3-cache%3A), [libs-milestone-s3-cache](http://repo.spring.io/webapp/browserepo.html?pathId=libs-milestone-s3-cache%3A) and [libs-release-s3-cache](http://repo.spring.io/webapp/browserepo.html?pathId=libs-release-s3-cache%3A).  These cache Spring project artifacts previously deployed to s3://maven.springframework.org. More on these below.
...
* [snapshot](http://repo.spring.io/snapshot) (aggregates _libs-snapshot-local_ and _libs-snapshot-s3-cache_)
* [milestone](http://repo.spring.io/milestone) (aggregates _libs-milestone-local_ and _libs-milestone-s3-cache_)
* [release](http://repo.spring.io/release) (aggregates _libs-release-local_ and _libs-release-s3-cache_)

Therefore, adding http://repo.spring.io/release as a repository within your build will ensure that you have seamless access to all Spring GA release artifacts, whether they were originally deployed against our older S3 repository or directly against repo.spring.io.

If you use http://maven.springframework.org URLs in your builds today, you'll be interested to know that DNS for maven.springframework.org now points to repo.spring.io.  These old repository urls (e.g. http://maven.springframework.org/snapshot) used to pull directly from S3, but they now pull from repo.spring.io.  There's nothing you need to change to adapt to this; you're free to keep these URLs in use (we'll preserve the DNS entry), but for clarity, we recommend switching to repo.spring.io for any new development.
...
* [libs-release](http://repo.spring.io/libs-release) (aggregates _release_, and all third-party GA _remote_ repositories)
* [libs-milestone](http://repo.spring.io/libs-milestone) (aggregates _milestone_ and _libs-release_)
* [libs-snapshot](http://repo.spring.io/libs-snapshot) (aggregates _snapshot_, _libs-milestone_, and all third-party snapshot _remote_ repositories)

These virtual repositories are quite powerful.  Declaring http://repo.spring.io/libs-release in your build script ensures that you can resolve *all* Spring project artifacts and downstream dependencies, but *only* GA versions of them.
...
# What are the benefits of the Spring repository?
This is not an exhaustive list of Artifactory benefits, but rather a select list of the benefits that Spring project teams and developers using Spring projects can expect to enjoy from using repo.spring.io.
...
For example, a custom Gradle [docbook-reference-plugin](https://github.com/SpringSource/gradle-plugins) has been developed to generate HTML and PDF reference documentation for Spring projects. This plugin is specifically tailored to meet the needs of Spring projects and is not a general-purpose utility.  Therefore, it need not live in Maven Central or any other repository.  The [plugin-snapshot-local](http://repo.spring.io/webapp/browserepo.html?pathId=plugins-snapshot-local%3A) and [plugin-release-local](http://repo.spring.io/webapp/browserepo.html?pathId=plugins-release-local%3A) repositories at repo.spring.io provide just the right kind of management for these artifacts.

Another example is hosting non-jar artifacts such as distribution, docs, and schema archives.  These could technically be stored in Maven Central, but repo.spring.io is a more natural home.  In addition, the Artifactory UI allows for browsing into these archives in-web, and even rendering HTML within them (as in the case of Javadocs or HTML reference docs).  This is simple and convenient and intuitive.
