...
* Spring MVC raises `NoHandlerFoundException` if there is no matching handler, and `ResponseStatusException(NOT_FOUND)` if there is no matching static resource, and also handles these with the aim of consistent handling for 404 errors out of the box, including RFC 7807 responses.
